...
if not exist "%PROJECT_FILE%" (
    echo Build_Project: Unrealtor_Demo.uproject not found in %PROJECT_DIR%
    pause
    exit /b 1
)
...
if not defined UE_ROOT (
    echo Build_Project: Could not locate Unreal Engine 5.6.
    echo.
...
if not exist "%BUILD_BAT%" (
    echo Build_Project: Build.bat not found at expected path.
    echo   %BUILD_BAT%
...
echo Build_Project: Engine root: %UE_ROOT%
echo.

echo Build_Project: Compiling Unrealtor_DemoEditor [Win64 ^| Development]...
echo.
call "%BUILD_BAT%" Unrealtor_DemoEditor Win64 Development -Project="%PROJECT_FILE%" -WaitMutex
set BUILD_EXIT=!ERRORLEVEL!

if !BUILD_EXIT! NEQ 0 (
    echo.
    echo Build_Project: Compilation failed [exit code !BUILD_EXIT!].
...
echo.
echo Build_Project: Build succeeded.
